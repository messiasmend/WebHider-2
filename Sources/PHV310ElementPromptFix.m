...
static NSString * const kPH310ElementSubtitle = @"Elemento Web selecionado";
...
static void PH310SetDetails(id self, NSString *details) {
    ...
    @try { [(PHInspectorViewController *)self setCurrentSubtitle:kPH310ElementSubtitle]; } @catch (__unused NSException *e) {}
}

static void PH310ShowSelected(id self, SEL _cmd, NSString *details) {
    // The original handler may rebuild the inspector and overwrite currentDetails.
    // Keep the actual selected payload, then render once more with that payload in place.
...
static void PH310Render(id self, SEL _cmd, BOOL hierarchyMode) {
    ...
    if (!hierarchyMode && saved.length) {
        // P233FinalUI consumes currentDetails synchronously while constructing the prompt.
        PH310SetDetails(self, saved);
    }
}
